...
class GoferRetry extends Gofer {
    constructor(config, hub) {
        super(config, hub);
        this.addOptionMapper(GoferRetry._retryMapper);
    }
    static _retryMapper(opts) {
    ...
    retry(endpoint, request, options, cb) {
        const op = Retry.operation(Object.assign({}, this.defaults.retry, (this.endpointDefaults[endpoint] && this.endpointDefaults[endpoint].retry), options.retry));
